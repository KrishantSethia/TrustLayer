...
class TranslationHandler(CategoryHandler):
    def validate_deliverable(self, text: str) -> None:
        ...
        if word_count < 20:
            raise ValueError(f"Translation too short (minimum 20 words, got {word_count})")
        if word_count > 15000:
            raise ValueError(f"Translation too long (maximum 15,000 words, got {word_count})")

    def get_decomposer_prompt(self, total_duration_days: int = 14) -> str:
        return f"""You are a senior project manager specializing in freelance translation projects.
Your task is to decompose a raw translation project requirement into a structured milestone roadmap.
...
- Use a progressive structure: terminology glossary / first pass translation, then section-by-section drafts, then final proofreading & consistency check
- ai_success_criteria must be concrete binary pass/fail checks specific to translation quality (accuracy, completeness, tone, terminology consistency, no omissions)
...
{{
  ...
  "milestones": [
    ...
    def get_judge_prompt(self) -> str:
        return """You are a strict translation quality assurance judge for a freelance escrow platform.
...
- Key translation quality checks: accuracy vs source, completeness (nothing omitted), fluency in target language, tone/register match, terminology consistency
